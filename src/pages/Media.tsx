...
export default function Media() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsLiveMode(false);
    try {
      const videos = await searchYouTube(query + ' worship sermon');
      setResults(videos);
      if (videos.length === 0) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="serif text-4xl font-bold text-sage-dark mb-2">Worship Sanctuary</h1>
          <p className="text-ink/60">A quiet place for praise, worship, and spiritual nourishment.</p>
        </div>
        <div className="flex gap-2">
          <button 
            onClick={() => setIsLiveMode(true)}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${isLiveMode ? 'bg-sage text-white shadow-lg shadow-sage/20' : 'bg-white text-sage-dark border border-sage/20'}`}
          >
